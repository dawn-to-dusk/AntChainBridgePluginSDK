...
private:
   ...
   void build_message_ordered(const name& invoker, const string& receiver_domain, const name& receiver_id, const string& msg, string& byte_array, uint32_t& send_seq);
   void build_message_unordered(const name& invoker, const string& receiver_domain, const name& receiver_id, const string& msg, string& byte_array, uint32_t& send_seq);
   void build_message_core(const uint32_t this_send_seq, const string& receiver_domain, const name& receiver_id, const string& msg, string& byte_array);

    void addcount(const name& invoker);
    uint64_t getcount(const name& invoker);
   eosio::checksum256 get_sdp_msg_key(string str1, string str2, string str3);
};
